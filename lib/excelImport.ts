...
// same conflict key (ma_noi_bo) in one statement — chỉ còn kiểm tra Mã nội bộ
// (mã hàng hóa), là trường duy nhất còn bắt buộc duy nhất; Mã vạch/Mã thùng
// không còn ràng buộc duy nhất nữa nên không cần kiểm tra trùng.
function checkDuplicates(rows: ProductRow[], field: "ma_noi_bo", label: string) {
